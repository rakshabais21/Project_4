...
public class FacultyModel {
	...
	public long add(FacultyBean bean) throws DuplicateRecordsException, SQLException{
		...
		CourseModel cmodel=new CourseModel();
		 ...
		 Bean=findByLoginId(bean.getEmailId());
			if(Bean!=null)
			{
				throw new DuplicateRecordsException("LoginId already exist..!!! ");
			}
			else
			{
	...
	public void update(FacultyBean bean){
		...
		Connection conn=null;
		StringBuffer sql= new StringBuffer("Update st_faculty set FIRST_NAME=?,LAST_NAME=?,GENDER=?,DOJ=?,QUALIFICATION=?,EMAIL_ID=?,MOBILE_NO=?,COLLEGE_ID=?,COLLEGE_NAME=?,COURSE_ID=?,COURSE_NAME=?,SUBJECT_ID=?,SUBJECT_NAME=?,CREATED_BY=?,MODIFIED_BY=?,CREATED_DATETIME=?,MODIFIED_DATETIME=? where ID=?");
